...
async function generateAkaNames() {
  try {
    ...
    services.forEach((service) => {
      ...
      // Extract different variations of the service name
      const akaNames = new Set<string>()

      const akasOfService = AKA[serviceName as keyof typeof AKA]
      if (akasOfService && akasOfService.length > 0) {
        akasOfService.forEach((aka) => {
          akaNames.add(aka)
        })
      }

      // Convert Set to array and add to map
      akaMap[serviceName] = Array.from(akaNames)
    })

    // Write the AKA names to file
      ...
      JSON.stringify(
        {
          ...
          description:
            'Generated AKA names for AWS services based on the provided service list.',
          data: akaMap,
        },
        null,
        2
    ...
    console.log('Successfully generated AKA names')

    // Print some statistics
    console.log(
      `Generated AKA names for ${Object.keys(akaMap).length} services`
    )
  } catch (error) {
    console.error('Error generating AKA names:', error)
    throw error
  }
}

generateAkaNames().catch(console.error)
...
const AKA = {
  ...
  'Amazon FSx': ['"File System performance"'],
  ...
  'Amazon VPC Lattice': ['Video Private Cloud Lattice'],
  'Amazon WorkDocs': [],
  'Amazon WorkMail': [],
  'Amazon WorkSpaces': [],
  ...
  'AWS Cloud Digital Interface SDK': ['CDI'],
  'AWS Cloud Map': [],
  'AWS Cloud9': [],
  'AWS CloudFormation': [],
  'AWS CloudHSM': [],
